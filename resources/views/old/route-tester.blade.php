    ...
    <div class="container-fluid py-4">
        ...
        <div class="row g-4">
            ...
            <div class="col-lg-5">
                ...
                <!-- Departure (route + matrix) -->
                <div class="card-glass p-4 mb-4" id="cardDeparture">
                    <div class="section-title">
                ...
                <div class="card-glass p-4 mb-4" id="waypointsCard">
                ...
                <div class="card-glass p-4 mb-4" id="cardDestination">
                    <div class="section-title">
                ...
                <div class="card-glass p-4 mb-4" id="cardMatrixDests" style="display:none;">
                    <div class="d-flex justify-content-between align-items-center mb-2">
                        <div class="section-title mb-0">
                            <i class="bi bi-geo-alt-fill text-danger me-1"></i> Destinations
                            <span class="badge bg-danger rounded-pill ms-2" id="matrixDestCount">1</span>
                        </div>
                        <button class="btn btn-sm btn-outline-danger rounded-pill" onclick="addMatrixDest()">
                            <i class="bi bi-plus-lg me-1"></i> Add
                        </button>
                    </div>
                    <div id="matrixDestsContainer">
                        <div class="matrix-dest-item" data-id="1">
                            ...
                            <div class="row g-2">
                                <div class="col-6"><input type="number" step="any" class="form-control coord-input" value="106.728272" placeholder="lng"></div>
                                <div class="col-6"><input type="number" step="any" class="form-control coord-input" value="-6.285317" placeholder="lat"></div>
                            </div>
                        </div>
                    </div>
                </div>

                <!-- Travel Mode & Options (route + matrix) -->
                ...
                <div class="card-glass p-4 mb-4" id="cardGeofence" style="display:none;">
                    ...
                    <div id="geoPane-put">
                        ...
                        <div id="geoPolygonInputs" style="display:none;">
                            ...
                            <button class="btn btn-sm btn-outline-warning rounded-pill" onclick="drawGeofenceOnMap()">
                ...
                <div class="card-glass p-4">
                    ...
                    <small class="text-muted d-block mt-2" id="mapHintMatrix" style="display:none;">
                        <i class="bi bi-info-circle me-1"></i>
                        First click = departure. Subsequent clicks add destinations.
                    </small>
    ...
    <script>
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    results.forEach((r, i) => {
                        ...
                        miniMap.addSource(srcId, { type: 'geojson', data: { type: 'Feature', properties: {}, geometry: { type: 'LineString', coordinates: [origin, dest] } } });
                        miniMap.addLayer({ id: srcId + '-line', type: 'line', source: srcId, paint: { 'line-color': color, 'line-width': 2.5, 'line-dasharray': [4, 3], 'line-opacity': 0.8 } });
                        matrixResultLayers.push(srcId, srcId + '-line');

                        // Label at midpoint
                        const midLng = (origin[0] + dest[0]) / 2, midLat = (origin[1] + dest[1]) / 2;
                        const el = document.createElement('div');
                        ...
                        const marker = new maplibregl.Marker({ element: el }).setLngLat([midLng, midLat]).addTo(miniMap);
        ...
        function renderLocResults(places, sub) {
            ...
            places.forEach((p, i) => {
                ...
                const placeIdBtn = p.placeId ?
                    `<button class="btn btn-sm btn-outline-secondary rounded-pill py-0 px-2 ms-auto" style="font-size:0.65rem;" onclick="event.stopPropagation(); useAsPlaceId('${p.placeId}')" title="Use as PlaceId"><i class="bi bi-arrow-right-short"></i> GetPlace</button>` :
                    '';
        ...
        function renderGeoResults(entries) {
            ...
            container.innerHTML = entries.length === 0 ? '<div class="text-center text-muted small py-3">No geofences</div>' :
                entries.map((e, i) => `<div class="geo-result-item" onclick="visualizeGeofenceData(${JSON.stringify(e).replace(/"/g, '&quot;')})">
                    <div class="d-flex align-items-center gap-2">
                        <span class="badge rounded-pill" style="background:#ffedd5;color:#ea580c;">${i + 1}</span>
                        <div class="flex-grow-1">
                            <div class="fw-semibold" style="font-size:0.82rem;">${e.GeofenceId || 'Unknown'}</div>
        ...
        function renderTrkResults(entries) {
            ...
            container.innerHTML = entries.length === 0 ? '<div class="text-center text-muted small py-3">No devices</div>' :
                entries.map((e, i) => `<div class="trk-result-item" onclick="miniMap.flyTo({center: [${e.Position?.[0]}, ${e.Position?.[1]}], zoom: 14})">
                    <div class="d-flex align-items-center gap-2">
