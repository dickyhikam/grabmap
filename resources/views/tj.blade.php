...
<body>
    ...
    <div class="sidebar-view">
        <div class="sidebar">
            ...
            <div class="sidebar-locations">
                ...
                <!-- Radio Buttons for Vehicle Type (Modern Tab Style) -->
                <div class="tabs">
                    ...
                    <input type="radio" id="vehicleTruck" name="vehicleType" value="Truck">
                    <label for="vehicleTruck" class="tab-label"><i class="fas fa-truck"></i> Truck</label>

                    <input type="radio" id="roadNormal" name="roadType" value="false" checked>
                    <label for="roadNormal" class="tab-label"><i class="fas fa-road"></i> Normal</label>

                    <input type="radio" id="roadToll" name="roadType" value="true">
                    <label for="roadToll" class="tab-label"><i class="fas fa-money-bill-wave"></i> Toll</label>
                </div>
                ...
                <div id="results_select" hidden></div>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const latlong = [106.87368787068785, -6.252809324296854];
        ...
        $(document).ready(function() {
            ...
            document.querySelectorAll('input[name="roadType"]').forEach((radio) => {
                radio.addEventListener('change', getSelectedRoad);
        ...
        function addMarker(latlong, address) {
            // Jika jumlah marker lebih dari 2, reset dan hapus semua marker
            if (window.markers.length >= 2) {
                // Menghapus semua marker dari peta
                window.markers.forEach(marker => marker.remove());
                window.markers = []; // Mengosongkan array markers

                map.getSource('route').setData({
                    type: 'Feature',
                    geometry: {
                        type: 'LineString',
                        coordinates: [] // Kosongkan koordinat saat reset
                    }
                });
            }
        ...
        function renderMarkerCard(markerData) {
            ...
            if (markerData.id == 1) {
                ...
                `;
                loc1.textContent = escapeHtml(markerData.address);
            } else {
                card.innerHTML = `
                    <div class="card-title">Lokasi ${markerData.id}</div>
                    <div class="card-address">${escapeHtml(markerData.address)}</div>
                ...
                `;
                loc2.textContent = markerData.address;
            }
        ...
        async function calculateRoute() {
            ...
            if (roadType == 'true') {
                params = {
                    "DeparturePosition": [start.lng, start.lat], // Koordinat titik keberangkatan
                    "DestinationPosition": [end.lng, end.lat], // Koordinat titik tujuan
                    "TravelMode": vehicleType, // Mode perjalanan (ganti dengan "Truck", "Walking", dsb.)
                ...
                params = {
                    ...
                    "TravelMode": vehicleType, // Mode perjalanan (ganti dengan "Truck", "Walking", dsb.)
                    "IncludeLegGeometry": true, // Sertakan geometri perjalanan
                    "AvoidTolls": false // Menghindari jalan tol
                };
            }
            ...
            try {
                ...
                if (data.Legs && data.Legs.length > 0) {
                    ...
                    routeDistanceEl.textContent = distanceInKm;
                    ...
                    // Hitung dan tampilkan jarak garis lurus
        ...
        async function calculateRouteMatrix() {
            ...
            if (roadType == 'true') {
                ...
                const params = {
                    DeparturePositions: starts.map(latlng => [latlng.lng, latlng.lat]), // Koordinat titik keberangkatan
                    DestinationPositions: ends.map(latlng => [latlng.lng, latlng.lat]), // Koordinat titik tujuan
                    TravelMode: vehicleType, // Mode perjalanan: 'Car'
                    IncludeLegGeometry: true, // Sertakan geometri perjalanan
                    AvoidTolls: true // Menghindari jalan tol
                };
            } else {
        ...
        // Fungsi untuk menghitung jarak garis lurus (Haversine Formula)
        function calculateStraightLD(lat1, lng1, lat2, lng2) {
            const R = 6371; // Radius bumi dalam KM
            const dLat = (lat2 - lat1) * Math.PI / 180;
